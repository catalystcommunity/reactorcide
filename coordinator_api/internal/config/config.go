...
var (
	...
	// CommitOnSuccess determines if transactions should be committed on
	// successful responses (2xx status) Default is true, but can be set to
	// false for testing environments
	CommitOnSuccess = env.GetEnvAsBoolOrDefault("REACTORCIDE_COMMIT_ON_SUCCESS", "true")

	// Corndogs integration: a CSIL-RPC-over-TCP address (host:port, no
	// scheme), or a comma-separated list of seed addresses for a corndogs
	// cluster (leader-following). Corndogs no longer speaks HTTP for RPC —
	// RPC is TCP (StreamCarrier) on CORNDOGS_LISTEN (default :5080); /healthz
	// and /metrics moved to the ops HTTP port CORNDOGS_HTTP_LISTEN (default
	// :8080).
	...
	// Default runner image for jobs that don't specify one Should be
	// configured per deployment (e.g.,
	...
	// SecretsLocalPath is the path for local secrets storage (only used when
	...
	// the container/pod. Mirrors the grace period described in. Not used for
	// kill (admin force-kill skips the grace period entirely).
