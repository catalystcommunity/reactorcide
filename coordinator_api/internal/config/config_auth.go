...
// UI auth mode values for REACTORCIDE_UI_AUTH_MODE.
const (
	UIAuthModeNone    = "none"
...
var (
	// UIAuthMode selects the management-UI login mode: "none" (no login, the
	// default), "local-rp" (DNS-less LinkKeys local-RP identity), or "rp"
	// (this coordinator is the app of a full LinkKeys RP deployment).
	...
	// LinkKeysRPFingerprints is a comma-separated set of pinned SPKI SHA-256
	// fingerprints for the RP server's TLS certificate (mirrors the RP's own
	// `_linkkeys` DNS TXT record). Required when UIAuthMode == "rp".
	...
	// LocalRPName is the display name (design doc: "app_name", audit/ display
	// metadata, never an identity input) used when generating this
	// coordinator's local-RP identity bundle. Required when UIAuthMode ==
	// "local-rp".
	...
	// BootstrapAdminToken, if set, allows a one-time bootstrap admin session
	// (see internal/auth.LoginService.BootstrapAdminSession) while zero
	// global admins exist, so initial setup can be done from the UI without a
	// LinkKeys login.
	...
	// LinkKeys login callback URL (UICallbackURL + "/app/auth/callback", the
	...
	// DefaultWorkerEnrollmentToken, if set, is a raw worker enrollment token
	// for the dev/alpha bootstrap:
	// postgres_store.PostgresDbStore.EnsureDefaultWorkerPool ensures a
	// "default" worker pool exists and registers this token's SHA-256 hash as
	// an active pool_enrollment_tokens row for it, idempotently (compared by
	// hash, never by re-reading a stored raw value — the raw value is never
	// persisted). This lets a dev/compose worker enroll with a token known in
	// advance instead of one minted through the admin UI. Never logged; read
	// only at startup.
