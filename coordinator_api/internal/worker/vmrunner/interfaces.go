// Package vmrunner implements the "vm" JobRunner backend: ephemeral,
// per-job guest VMs used to run native macOS/Windows jobs on host workers
// (see VM_RUNNERS_PLAN.md at the repo root for the full design).
//
...
// channel, and image resolution -- is cross-platform and testable on
// Linux:
//
...
// coordinator/lease/store-shaped): internal/worker/runner_factory.go needs
// to import vmrunner to construct the "vm" backend, so vmrunner importing
// worker back would be a cycle. JobConfig here is vmrunner's own mirror of
...
// VMRunner is a pure JobRunner-shaped type usable identically from
// run-local and the coordinator-mediated worker.
package vmrunner
...
// content (a registry digest, or file identity for a local path) so
// repeated Resolve calls for the same ref are cheap.
type ImageSource interface {
	// Resolve pulls/caches the base image referenced by imageRef and
	// returns its local path. imageRef is whatever JobConfig.Image the job
	// specifies -- for LocalImageSource that's a file path (absolute, or
	// relative to a configured base dir); VM-2's OCI ImageSource will treat
	// it as an OCI artifact ref.
...
type BootSpec struct {
	// CPUs is the number of virtual CPUs to give the guest. 0 means "let
	// the lifecycle choose a default".
	CPUs int
	...
	// correlation -- the VM-equivalent of DockerRunner's
	// "reactorcide.job_id" container label.
	JobID string
...
// GuestAddr is how the host reaches a booted guest's transport endpoint.
// Both fields are required for any transport dialed over TCP (SSH today);
// a future vsock-based transport could repurpose Host as a CID.
type GuestAddr struct {
	Host string
	Port int
}

// VMLifecycle clones a base image and boots/destroys the resulting guest
// VM. It is the one seam that is genuinely platform-specific (Apple
// Virtualization.framework / Hyper-V) and is therefore build-tag selected
// via newVMLifecycle() rather than constructed directly by callers.
type VMLifecycle interface {
	...
	// VMRunner may call it from more than one path (a failed boot's
	// cleanup, Stop's grace timeout, and Cleanup can race in principle).
...
// GuestTransport starts a command inside an already-booted guest and
// streams its output back. SSH (guest_ssh.go) is the VM-1 prototype
// implementation; VM_RUNNERS_PLAN.md notes vsock as a possible future
...
type GuestTransport interface {
	...
	// -- see JobConfig.Env; implementations must never log env values).
	// Start itself should return promptly once the command has begun
	// running in the guest; callers use the returned GuestSession to wait
	// for completion.
	Start(ctx context.Context, addr GuestAddr, creds GuestCreds, cmd []string, env map[string]string) (GuestSession, error)
}

// GuestSession represents one running guest command. Wait, Stdout, and
// Stderr are all safe to use concurrently (e.g. StreamLogs's readers next
// to a separate WaitForCompletion caller), and Wait is safe to call more
// than once (implementations must memoize the result) since both
// VMRunner.Stop's grace check and WaitForCompletion may call it.
type GuestSession interface {
	...
	// name without the "SIG" prefix, e.g. "TERM", "KILL"). Best-effort:
	// see guest_ssh.go's doc comment for why the SSH implementation also
	// falls back to an explicit `kill` rather than relying solely on the
	// SSH protocol's signal request.
