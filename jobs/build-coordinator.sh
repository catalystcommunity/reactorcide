...
# Build and push coordinator-api image to registry
# Usage: ./build-coordinator.sh [tag]

# Registry configuration - override with environment variables
REGISTRY="${REACTORCIDE_REGISTRY:-containers.catalystsquad.com}"
IMAGE="${REACTORCIDE_IMAGE:-public/reactorcide/coordinator}"
TAG="${1:-dev}"
...
docker build -t "$REGISTRY/$IMAGE:$TAG" -f Dockerfile.coordinator .
