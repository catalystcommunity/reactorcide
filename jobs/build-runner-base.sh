...
# Build and push runnerbase image to registry
# Usage: ./build-runner-base.sh [tag]

# Registry configuration - override with environment variables
REGISTRY="${REACTORCIDE_REGISTRY:-containers.catalystsquad.com}"
IMAGE="${REACTORCIDE_IMAGE:-public/reactorcide/runnerbase}"
TAG="${1:-dev}"
...
docker build -t "$REGISTRY/$IMAGE:$TAG" -f Dockerfile.runner .
