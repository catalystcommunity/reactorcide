...
def create_api_token(config: Dict[str, Any], dry_run: bool = False) -> None:
    """Create API token and store in the default user secret.

    The helm chart creates a secret (defaults.userSecretName, default 'base-reactorcide-user')
    with a generated user-id. This function checks if a token already exists in that secret.
    If not, it creates one and updates the secret.
    """
    log("Creating API token")

    namespace = config['namespace']
    release = config['release']
    user_secret_name = config['user_secret_name']
    ...
    try:
        existing_token = run_cmd_output(
            f"kubectl get secret {user_secret_name} -n {namespace} "
            f"-o jsonpath='{{.data.token}}' 2>/dev/null"
        )
        if existing_token:
            log(f"API token already exists in secret {user_secret_name}, skipping creation")
            return
    except subprocess.CalledProcessError:
        pass  # Secret doesn't exist or no token field - continue to create

    # Get user ID from the secret (generated by helm chart)
    try:
        user_id_b64 = run_cmd_output(
            f"kubectl get secret {user_secret_name} -n {namespace} "
            f"-o jsonpath='{{.data.user-id}}'"
        )
        if not user_id_b64:
            log(f"WARNING: No user-id found in secret {user_secret_name}")
            return
        # Decode base64
        import base64
        default_user_id = base64.b64decode(user_id_b64).decode('utf-8')
    except subprocess.CalledProcessError as e:
        log(f"WARNING: Failed to get user-id from secret {user_secret_name}: {e}")
        return
    ...
    if not coordinator_pod:
        log("WARNING: Could not find coordinator pod for token creation")
        return
    ...
    try:
        ...
        if api_token:
            ...
            run_cmd(
                f"kubectl patch secret {user_secret_name} -n {namespace} "
                f"--type=merge -p '{{\"data\":{{\"token\":\"{token_b64}\",\"token-id\":\"{token_id_b64}\"}}}}'"
            )
            log(f"API token stored in secret: {user_secret_name}")
        else:
...
def deploy(config: Dict[str, Any], dry_run: bool = False) -> int:
    ...
    log("=" * 50)
    log("")
    user_secret = config['user_secret_name']
    log("Retrieve API token:")
    log(f"  kubectl get secret {user_secret} -n {namespace} -o jsonpath='{{.data.token}}' | base64 -d")
    log("")
